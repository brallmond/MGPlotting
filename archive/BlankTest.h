...
// Fri Sep  9 11:02:07 2022 by ROOT version 6.26/06
// from TTree LHEF/Analysis tree
// found on file: sm_events.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   //BlankTest(TTree *tree=0);
   BlankTest(TString name);
   virtual ~BlankTest();
...
#ifdef BlankTest_cxx
//BlankTest::BlankTest(TTree *tree) : fChain(0) 
BlankTest::BlankTest(TString name)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   TTree* tree = nullptr;
   if (tree == 0) {
      //TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("sm_events.root");
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(name);
      if (!f || !f->IsOpen()) {
         //f = new TFile("sm_events.root");
         f = new TFile(name);
      }
...
Int_t BlankTest::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t BlankTest::LoadTree(Long64_t entry)
{
...
void BlankTest::Init(TTree *tree)
{
...
void BlankTest::Show(Long64_t entry)
{
...
Int_t BlankTest::Cut(Long64_t entry)
{
